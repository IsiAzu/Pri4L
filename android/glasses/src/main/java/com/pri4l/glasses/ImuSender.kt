...
/**
 * Publishes the glasses' IMU to /glasses/imu at ~10Hz.
 *
 * Coordinate frame: Android x-right, y-up, z-out-of-screen — no transform applied; the hub
 * converts when consuming. frame_id is "glasses_imu" to distinguish from the phone client.
 */
class ImuSender(
    ...
    private val accelListener = object : SensorEventListener {
        override fun onSensorChanged(event: SensorEvent) { accel = event.values.clone() }
        override fun onAccuracyChanged(sensor: Sensor?, accuracy: Int) {}
    }

    private val gyroListener = object : SensorEventListener {
        override fun onSensorChanged(event: SensorEvent) { gyro = event.values.clone() }
        override fun onAccuracyChanged(sensor: Sensor?, accuracy: Int) {}
    }
        ...
        override fun run() {
            ...
            val msg = JSONObject().apply {
                put("header", JSONObject().apply {
                    ...
                    put("frame_id", "glasses_imu")
                })
                put("linear_acceleration", JSONObject().apply {
                    put("x", accel[0].toDouble()); put("y", accel[1].toDouble()); put("z", accel[2].toDouble())
                })
                put("angular_velocity", JSONObject().apply {
                    put("x", gyro[0].toDouble()); put("y", gyro[1].toDouble()); put("z", gyro[2].toDouble())
                })
                put("orientation", JSONObject().apply {
                    put("x", 0.0); put("y", 0.0); put("z", 0.0); put("w", 0.0)
                })
                put("orientation_covariance", JSONArray().apply { put(-1.0); repeat(8) { put(0.0) } })
                put("angular_velocity_covariance", JSONArray().apply { repeat(9) { put(0.0) } })
                put("linear_acceleration_covariance", JSONArray().apply { repeat(9) { put(0.0) } })
            }

            client.publish("/glasses/imu", "sensor_msgs/msg/Imu", msg)
